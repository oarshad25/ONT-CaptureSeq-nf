...
11. Calculate CaptureSeq metrics:
    ...
    * Genes of interest (GOI) stats: Number of reads, N50 read length and count of reads spanning the annotated start and end of the longest transcript for reads overlapping each gene of interest locus.
...
| `panel_gene_list`  | string | Path to file of list of genes in capture target panel, one gene (gene id or name) per line |  |
| `goi_gene_list`  | string | Path to file of list of a few genes of particular interest (GOI), one gene per line (gene name or ID), whose read stats are to be included in report generated by pipeline. Must be a subset of panel gene list. |  |
