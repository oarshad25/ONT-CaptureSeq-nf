...
11. Calculate CaptureSeq metrics:
    * Calculate counts of primary reads overlapping capture-panel gene-body intervals (on-target) and off-target counts.
    * Calculate exon coverage [mosdepth](https://github.com/brentp/mosdepth) over merged non-overlapping exonic intervals seperately for genes in capture panel and for genes of interest (GOI).
    * GOI stats: Number of reads, N50 read length, count of reads spanning the annotated start and end of the longest transcript, mean exonic depth, and exonic breadth for each gene of interest.
12. Summarise QC stats ([MultiQC](https://multiqc.info/docs/)).
13. Optionally quantify transcript abundance with [Oarfish](https://combine-lab.github.io/oarfish/) when a transcriptome FASTA is provided.
...
| `transcriptome` | string | Optional transcriptome FASTA used to enable transcript quantification with Oarfish. If unset, Oarfish is skipped. | |
| `panel_gene_list`  | string | Path to file of list of genes in capture target panel, one gene (gene id or name) per line. Used for gene-body on-target counts and panel exon coverage. |  |
| `goi_gene_list`  | string | Path to file of list of a few genes of particular interest (GOI), one gene per line (gene name or ID), whose read support and exon coverage stats are to be included in the report generated by the pipeline. Must be a subset of panel gene list. |  |
...
| `minimap2_u`                   | character | Minimap2 `-u` option e.g. `"b"`. If provided, value supplied is used. If this parameter isn't provided and `run_cdna_qc` flag is set (pipeline default) then reads are reoriented during cdna read QC and hence this argument is set to `"f"` (forward strand), otherwise Minimap2 default (`"n"`) is used    | `"f"` if `run_cdna_qc` else Minimap2 default |
...
Additional parameters for tuning calculation of CaptureSeq metrics.

| Parameter                  | Type    | Description                                                                       | Default      |
| -------------------------- | ------- | --------------------------------------------------------------------------------- | ------------ |
| `goi_fulllength_margin`   | integer | Configurable margin for calculation of the count of reads per gene of interest (GOI) whose aligned reference span covers the annotated start and end of the longest transcript. This allows reads to miss the transcript start/end by up to a certain number of aligned reference bases while still being counted. | 250       |

#### Oarfish transcript quantification

Oarfish is run only when `transcriptome` is provided. The pipeline builds one reusable Oarfish/minimap2 transcriptome index, quantifies each sample from the QC'ed FASTQs, and writes per-sample `.quant`, `.meta_info.json`, and `.ambig_info.tsv` outputs under `oarfish/<sample>/`. It also writes a merged transcript x sample matrix to `oarfish/oarfish.transcript_counts.tsv` using the Oarfish `num_reads` column. Missing transcript/sample combinations are left empty, while explicit zero counts remain `0`.

| Parameter | Type | Description | Default |
| --------- | ---- | ----------- | ------- |
| `oarfish_seq_tech` | string | Oarfish `--seq-tech` option. Valid values are `"ont-cdna"`, `"ont-drna"`, `"pac-bio"`, and `"pac-bio-hifi"`. | `"ont-cdna"` |
| `oarfish_filter_group` | string | Oarfish `--filter-group` option. Valid values are `"no-filters"` and `"nanocount-filters"`. | `"no-filters"` |
| `oarfish_model_coverage` | boolean | Whether to pass `--model-coverage` to Oarfish. | true |
| `oarfish_extra_opts` | string | Any additional command line options to pass to Oarfish quantification. | |
