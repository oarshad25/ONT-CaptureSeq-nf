...
Computes read count and read length N50 for each gene of interest (GOI)
region from an aligned BAM file.
...
Outputs a single wide-format TSV with one row per sample and paired
read_count_<gene> and n50_<gene> columns for each GOI, suitable for
ingestion by goi_combine_mqc.py.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Count primary mapped reads and compute N50 of read lengths per GOI region"
    )
...
def main():
    ...
    for chrom, start, end, gene in goi:
        ...
        results.append((gene, count, n50))

    # Write wide-format TSV — one row per sample, paired columns per GOI:
    # read_count_<gene> followed immediately by n50_<gene> for each gene
    with open(args.output, "w") as out:
        out.write(
            "Sample\t"
            + "\t".join(f"read_count_{g}\tn50_{g}" for g, _, _ in results)
            + "\n"
        )
        out.write(
            args.sample + "\t"
            + "\t".join(f"{c}\t{n}" for _, c, n in results)
            + "\n"
        )
